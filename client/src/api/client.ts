...
async function request<T>(path: string, method: HttpMethod, body?: unknown): Promise<T> {
    console.log(`API Request: ${import.meta.env.VITE_API_BASE} ${path}`, body);
    let res = await fetch(`${import.meta.env.VITE_API_BASE}${path}`, {
        method,
        credentials: "include",
        headers: {
...
export async function handleTokenRefresh<T>(
    ...
    try {
        const refreshRes = await fetch(`${import.meta.env.VITE_API_BASE}/auth/refresh`, {
            method: "POST",
            credentials: "include",
        });
